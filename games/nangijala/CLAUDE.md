...
- **Tile self-emission** is data-driven from `tiles/emission.json`
  ...
  sync); (2) clustered SHADOW-FREE glow pools (ShaderLight with **negative
  radius** = no LOS march, no wall gating; emissive cells with exposed faces
  add pool samples floating IN FRONT of the face at mid-height so glow
  reaches the base ground + neighbouring walls); (3) per-pixel GLOW HALOS —
