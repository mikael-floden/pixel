...
await page.fill("input", "wf-lavaprobe-" + Math.floor(Math.random() * 1e5));
...
async function snap(name) {
  await page.waitForTimeout(700); // let idle anim settle
  ...
  console.log(name, "me=", JSON.stringify({ x: info.me.x / 32, y: info.me.y / 32 }));
  console.log(name, "depthProbe=", JSON.stringify(info.probe));
}

// approach waypoint from NW
console.log("wp1:", JSON.stringify(await walkTo(259.5, 220.5)));

// (a) beside-west
console.log("a:", JSON.stringify(await walkTo(260.4, 222.6)));
await snap("a-beside-west");

// (b) close behind-NW
console.log("b:", JSON.stringify(await walkTo(260.8, 221.9)));
await snap("b-behind-nw");

// (c) beside-east via [262.5,220.5]
console.log("wp-c:", JSON.stringify(await walkTo(262.5, 220.5)));
console.log("c:", JSON.stringify(await walkTo(262.6, 222.4)));
await snap("c-beside-east");

// (d) front-south via [262.5,223.5]
console.log("wp-d:", JSON.stringify(await walkTo(262.5, 223.5)));
console.log("d:", JSON.stringify(await walkTo(261.5, 223.4)));
await snap("d-front-south");
