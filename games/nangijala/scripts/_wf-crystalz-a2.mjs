...
await page.fill("input", "wf-crystalz-a2-" + Math.floor(Math.random() * 1e5));
...
// Approach from due north so the greedy controller never touches the pillar:
// spawn ~(260,225) -> north lane row 223.5 -> above the pillar -> drop south to the pose.
for (const [c, w] of [[261.5, 223.5], [263.4, 224.0], [263.4, 225.5]]) {
  const res = await walkTo(c, w);
  console.log("wp", c, w, "->", JSON.stringify(res));
}
await page.waitForTimeout(600);
const clip = { x: 700 - 225, y: 425 - 235, width: 450, height: 470 };
await page.screenshot({ path: `${SHOT}/wf-crystalz-a2-behind-north.png`, clip });
