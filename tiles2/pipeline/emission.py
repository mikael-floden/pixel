...
# REAL-LIGHT overrides (games2 light-slot system, 2026-08-12). A slotted
# emissive prop becomes a true shader point light — attenuation, LOS shadows,
# elevation — under a 12-slot budget (8 for the world; see
# games2/spec/LIGHT_BUDGET.md). Keyed by tile-path STEM (no extension, wins)
# or material name; None = "stamp only, never a real light". Fields, all
# optional: radius (CELLS, cap 7), color [r,g,b] (values >1 = overbright: the
# shader clamps the multiply at 1.25, so excess WIDENS the hot plateau — the
# campfire trick), flicker 0..1, shadows (False = shadow-free glow pool),
# z (levels above the cell surface). Absent -> games2 derives a QUIET default
# from the material + sources, so a curated entry is for tiles that should
# LEAD a scene, not for every glowing pebble.
LIGHTS = {
    # The bonfire tile — pinned to the campfire scenery object's EXACT light
