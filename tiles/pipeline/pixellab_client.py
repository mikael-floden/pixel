...
# The tile CDN rejects urllib's default User-Agent ("Python-urllib/3.11") with a
# bare 403 — any ordinary UA is accepted. Without this every download of a finished
# sheet fails while the API calls themselves succeed, which reads as "the generation
# produced nothing" rather than as a download problem.
_UA = "Mozilla/5.0 (compatible; nangijala-tiles/3.0)"


def _fetch_image(url, retries=4):
    """GET one tile image -> PIL. Retries: a freshly-written CDN object can 404 for
    a moment right after the job completes."""
    import io
    import time
    import urllib.error
    ...
    for i in range(retries):
        try:
            ...
            return _Image.open(io.BytesIO(raw)).convert("RGBA")
        except Exception:
            if i == retries - 1:
                return None
            time.sleep(2 * (i + 1))
...
class PixelLabClient:
    ...
    def create_tileset(self, lower_description, upper_description, transition_description=None,
                       tile_size=64, view="high top-down", mode="pro", shape_style=None,
                       spread_x=0.5, slope_size=0.0, raggedness=0.0, transition_size=0.0,
                       outline=None, seed=None, job_timeout=600):
        """Generate a Wang TRANSITION set between two terrains in ONE call — the
        tiles 3.0 replacement for tiles2's per-pair, per-direction transition sheets.

        The pro pipeline exposes the geometry directly, which is what makes this
        controllable instead of a lottery:
          * `spread_x`     0 = steep cliff, 1 = gradual — how far the drop spreads sideways.
          * `slope_size`   slope on N/W/E as a fraction of wall height; > 0 is what makes
                           a tile WALKABLE from one level to the next instead of a jump.
          * `raggedness`   0 = smooth/straight boundary, 1 = rough. Tiles connect either way.
          * `transition_size` width of the blended band (0, 0.25, 0.5, 1.0).
        `upper` is the raised plateau's top surface, `lower` the ground it falls to.
        """
        payload = {
        ...
        if shape_style:
            payload["shape_style"] = shape_style
        ...
        return last, resp.get("tileset_id") or last.get("tileset_id")
    ...
    def create_tiles(self, description, tile_size=64, tile_view="high top-down",
        ...
        # THE tiles 3.0 default. 'outline' (the API default, and all of tiles2) bakes a
        # dark line around every tile — tiles2 spent months fighting that line with
        # post-processing (neutralize_outline, deseam, fade_outline, clean_top_rim) and
        # never fully won. 'segmentation' generates colour zones with no outline at all,
        # which is both cleaner AND a precondition for a 100%-flat single-colour top.
        ...
        # This endpoint does not always inline the art in the job result — it stores
        # it and returns only `storage_urls` on the tile record. Falling back to a
        # fetch-by-id means a completed (already PAID FOR) generation is never lost
        # just because the job payload was thin.
        return (self.fetch_tiles(tile_id) if tile_id else []), tile_id

    def fetch_tiles(self, tile_id):
        """Download a completed tiles-pro sheet by id -> [PIL, ...] in tile order.

        Costs nothing (the generation is already paid for), so it doubles as the
        recovery path for an interrupted run and as the way to pull sheets that were
        generated in the PixelLab UI.
        """
        rec = self._get(f"/tiles-pro/{tile_id}")
        urls = rec.get("storage_urls") or {}
        out = []
        for k in sorted(urls, key=lambda s: int(s.rsplit("_", 1)[1])):
            out.append(_fetch_image(urls[k]))
