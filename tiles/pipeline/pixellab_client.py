"""PixelLab API client for the TILES domain — isometric tile generation.

The tiles agent generates focused sets of isometric terrain tiles via the
`create-tiles-pro` endpoint (tile_type=isometric). That endpoint is asynchronous:
POST returns {tile_id, background_job_id}; poll the job, then the completed job's
`last_response.images` holds the tiles as RAW RGBA bytes (base64) with width/height
(NOT PNG). This client hides all that — `create_tiles` blocks and returns decoded
Pillow images.
