...
# NO SLOPE MODE. The published slope library is retired (maintainer 2026-09-26: "The game
# agent has invented a dynamic way to do slope tiles so you can remove your slope tiles");
# games2 composes a ramp from the cell's OWN member plate (`syntheticRampSet`,
# `buildRampPixels`), so a one-level rise is walkable on every ground with no art to buy.
# This module generates the transition matrix and nothing else.
...
def pairs():
    ...
    NO SELF-PAIRS. Flat, a set between a material and itself has no boundary to draw.
    Raised it was a plateau, which is what the retired slope library bought; the game
    composes that from the plate now, so x-to-x has nothing left to generate.
    """
...
run() {  # a b amp seed elevation step_slope description
  ...
  for TRY in 1 2 3 4 5; do
    ...
    if [ -n "$ID" ]; then
      ...
      # THE FIRST JOB IS A PROBE. A rejected parameter value fails identically for
      # every job in the list - a wasted trip for someone driving this from a phone.
      # With nothing yet recorded, stop and print what the API actually said, which
...
if __name__ == "__main__":
    ...
    if a.shell:
        path = os.path.join(out_dir, "run_in_cloudshell.sh")
        # THE HOUSE PERSPECTIVE, ALWAYS: every tile takes GEOMETRY's tile_depth_ratio.
        # A run that copied the web UI's Thickness 0% produced flat-top-only art - the
        # diamond correct (widest@row14, 2.21px/row, 2px flat top) but with no wall it
        # reads as a low-res top-down chip. Maintainer, 2026-08-28: "You generated low
        # res topdown. I wanted the perspective we always have and 64x64."
        depth = a.depth_ratio
        open(path, "w").write(shell_script(
            jobs, done="run_done.txt", depth_ratio=depth) + "\n")
