...
/** LIGHT-TOUCH BORDER RING (maintainer round 11, "take a step back"): the
 * frame ART is the mock's ORIGINAL pixels, untouched — every earlier attempt
 * to snap/re-synthesize the art at cell resolution (solidified blocks,
 * majority votes, straightened rails) made the border more perfect and the
 * frame less like the reference, until the whole HUD read chunky and wrong.
 * The only baked addition is the border the maintainer approved in round 8:
...
function outline(img, ox = 0, oy = 0, alpha = 230) {
  ...
  // A block is ART when a real share of it is opaque (soft-brush fringe
  // below a quarter stays plain background).
  ...
  // Ring: blocks that ARE art (their empty remainder hugs the art) or
  // 8-touch art. Flat colour per block from its own TRANSPARENT px (page
  // navy — never tinted by the art's gold), faded per the round-8 recipe.
  for (let by = 0; by < bh; by++)
    for (let bx = 0; bx < bw; bx++) {
      ...
      if (!n) continue; // fully solid art block — nothing to ring
      r = Math.round((r / n) * 0.85);
      g = Math.round((g / n) * 0.85);
      b = Math.round((b / n) * 0.85);
      for (const [x, y] of px) {
        const o = (y * w + x) * 4;
        if (data[o + 3] >= alpha) continue; // real art stays untouched
        data[o] = r;
        data[o + 1] = g;
        data[o + 2] = b;
        data[o + 3] = alpha;
