...
/** THE CROSSING IS A HAND-OVER, NOT A DISAPPEARANCE (maintainer 2026-09-21,
 *  seven screenshots at the four-zone cross 297.2,96.4: "when they cross a
 *  zone they often disappear and appear again"). A client watching a monster
 *  walk over a border must never lose the id: the room it leaves holds it as
 *  its own ghost the moment it hands it on, the room it enters puts the new
 *  instance into every view that already held the id, and an orphaned ghost
 *  gets GHOST_ORPHAN_MS to be claimed instead of being deleted outright.
 *  Before that the delete shipped at once and the replacement waited for the
 *  next interest pass — measured at his corner, a 51 ms hole on a
 *  neighbour-to-neighbour crossing and up to 550 ms while the rooms idled —
 *  so the sprite was destroyed and rebuilt every time. Sampled at 10 ms, this
 *  is what a client sees. */
test("a monster crossing a border is never missing from the client's view", async (t) => {
  if (!HAVE_WORLD) return t.skip(SKIP);
  const port = 2962; // unique per test FILE — see test/ports.test.ts (2977 is combat.review.test.ts's)
  useBus(new FakeBus());
  resetWorldClocks();
  const gameServer = new Server({ transport: new WebSocketTransport({ server: createServer() }) });
  gameServer.define(ROOM_NAME, WorldRoom).filterBy(["world", "zone"]);
  await gameServer.listen(port);
  // The watcher's own interest radius is the real one: the hole this gate is
  // about is the interest pass, so switching interest off would hide it.
  const base = { world: "the_game", zonesCfg: CFG, monsterSeed: 11 };
  try {
    const cA = new Client(`ws://localhost:${port}`);
    const cB = new Client(`ws://localhost:${port}`);
    const rA: any = await cA.joinOrCreate(ROOM_NAME, { ...base, zone: 0, name: "Watcher", character: "default_boy" });
    const rB: any = await cB.joinOrCreate(ROOM_NAME, { ...base, zone: 1, name: "Pusher", character: "default_girl" });
    rA.onMessage("zone:go", () => {});
    rB.onMessage("zone:go", () => {});
    const y = 100 * CELL_WU;
    // The watcher stands three cells from the line and never moves: everything
    // below happens inside his interest square, on both sides of the border.
    // INTEREST IS ON, so park the bodies BEFORE waiting for monsters: at the
    // world spawn a client's view holds none of them.
    rA.send("teleport", { x: BORDER_X - 3 * CELL_WU, y });
    rB.send("teleport", { x: BORDER_X + 3 * CELL_WU, y });
    await waitFor(() => rA.state.monsters.size > 0 && rB.state.monsters.size > 0, BUS_MS, "both zones seeded and in view");
    await settle(500);
    // One of zone 0's monsters, parked two cells inside the border so the
    // watcher holds it, then pushed one cell over the line.
    const [mid] = [...rA.state.monsters.entries()][0] as [string, any];
    rA.send("dbgmonster", { id: mid, x: BORDER_X - 2 * CELL_WU, y, pin: true });
    await waitFor(() => (rA.state.monsters.get(mid)?.x ?? 0) < BORDER_X, BUS_MS, "parked by the line");
    await settle(400);
    assert.ok(rA.state.monsters.has(mid), "the watcher holds the monster before the crossing");

    // Sample what the WATCHER'S CLIENT holds, every 10 ms, across the crossing.
    let gaps = 0;
    let worst = 0;
    let missingSince = 0;
    const sample = () => {
      const here = rA.state.monsters.has(mid) || rA.state.ghostMonsters?.has(mid);
      const now = Date.now();
      if (here) {
        if (missingSince) { worst = Math.max(worst, now - missingSince); missingSince = 0; }
      } else if (!missingSince) { missingSince = now; gaps++; }
    };
    const iv = setInterval(sample, 10);
    rA.send("dbgmonster", { id: mid, x: BORDER_X + CELL_WU, y, pin: true });
    await waitFor(() => rB.state.monsters.has(mid), BUS_MS, "the monster changed owner");
    await settle(1500);
    clearInterval(iv);
    if (missingSince) worst = Math.max(worst, Date.now() - missingSince);
    assert.ok(rA.state.monsters.has(mid) || rA.state.ghostMonsters?.has(mid), "the watcher still holds it after the crossing");
    assert.equal(
      gaps,
      0,
