...
const AH = 12; // art pixels down (the pill is deliberately short)
const SCALE = 2; // 1 art px = 2 css px
const HOR = 8; // horizon row: where the orbs cross the hills
const AMP = 6; // arc height in art px
...
const SPOTS: [number, number][] = [
  [3, 2], [9, 1], [15, 3], [22, 1], [28, 4], [34, 2], [37, 5], [6, 5],
];
// Three paper planes: the first cut as peaks, the others as gentle waves.
const LAYERS = [
  { b: 7, a: 1.6, f: 0.24, o: 0, peak: true },
  { b: 9, a: 1.2, f: 0.19, o: 2, peak: false },
  { b: 11, a: 0.8, f: 0.22, o: 4, peak: false },
];
...
function sun(cx: number, cy: number) {
  disc(cx, cy, 2.8, SUN_C);
  ring(cx, cy, 2.8, SUN_A);
}
function moon(cx: number, cy: number) {
  disc(cx, cy, 2.8, MOON_C);
  px(cx - 1, cy - 1, MOON_A);
  px(cx + 1, cy + 1, MOON_A);
  px(cx + 1, cy - 2, MOON_A);
}
...
function paint(u: number) {
  ...
  if (n > 0.05) {
    for (const [x, y] of SPOTS) px(x, y, STAR_C, (n * 220) | 0);
    const ph = (u * 4) % 1;
...
function mount() {
  ...
  style.textContent = `
  .ml-clock{position:fixed;top:8px;left:50%;transform:translateX(-50%);z-index:8;
