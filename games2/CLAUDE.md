...
- **Asset loading is SPLIT + deploy-pinned cached** (maintainer 2026-07-29:
  ...
  **THE BUNDLE IS A SEPARATE GRANT, AND IT IS NOT THE SAME QUESTION** (fixed
  2026-08-15). Everything rollup emits into `client/dist/assets` is named
  `<name>-<contenthash>.<ext>`, so those URLs are immutable by construction —
  no sha comparison needed. The rule used to match `js|css` only, which left
  **532 of the 535** emitted files (503 `.ogg` foley takes, 13 `.m4a` + 2
  `.mp3` beds, 5 `.wav`, 9 `.webp`) on `no-cache`: 532 revalidation round
  trips on every repeat visit, covering ~11 MB of bundled audio. The grant is
  scoped **BY DIRECTORY**, not by filename shape, and that distinction is the
  whole safety argument: `setCacheHeaders` also serves the `/assets/<domain>`
  ART mounts, where agents repaint tiles IN PLACE, so a filename-shape rule
  would freeze a repainted tile for a year the first time an art file was
  named `foo-a1b2c3d4.webp` (measured: zero match today — luck, not an
  invariant). A file inside `client/dist/assets` is rollup output by
  construction; `client/public` has no `assets/` directory, so nothing else
  can land there. Policy + rationale in `server/src/cachepolicy.ts`, a pure
  function precisely so it is testable without booting a server; the
  regression cases are `server/test/cachepolicy.test.ts`. This does NOT
  contradict the paragraph above — that one forbids per-file hashes for ART,
  whose paths are hand-written and edited in place. The bundle's hashes are
  generated by the bundler from the bytes, which is the property the art
  paths lack.
