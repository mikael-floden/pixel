...
- **EVERY VISIBLE EDGE WEARS A 2 PX OUTLINE, EACH LINE ONE TEXEL WIDE AND
  ...
  - A BRIDGE, A CAVE LID OR A ROOF IS THE GROUND CONTINUING (his, 2026-09-26:
    "if a player can walk straight onto a roof and they have the same height
    it is the same ground"): `edgeSet` reads a
    neighbour carrying a deck as the surface nearest its own corner, so a bank
    meeting a slab at its level draws no line; every slab top wears one on each
    side over lower ground (`deckTop`, the
...
- **A ONE-LEVEL RISE IS A RAMP THE BODY WALKS UP; A CLIFF OF TWO KEEPS ITS
  ...
  every level, and keeps the whole incline (`withBand` false, one margin
...
- **A STUMP'S LID IS THE ROOF'S GROUND OVER IT, ELSE THE ROCK IT IS CUT
  ...
  and a change repaints through `reResolve`. THE LID IS DRAWN AT ITS OWN
  SIZE (maintainer 2026-09-26: "we might be able to draw tiles in their
  correct size now without adding 1px border"): re-cut to the periodic
  outline, the union of lids is one straight staircase in exact maths. (It
  was grown a pixel on every side and a second up and right for his dotted
  seams at 331.8,233.6 / 205.6,217.5 and "still 1px too small so it jitters
  when I walk" at 308.0,228.7 — Phaser's 16-bit vertex maths on his Mali,
  which every shader compiling highp ends (`docs/perf.md`). Headless without
  the growth at those spots: 0 texels of the page between lids; only the
  lid's own edge moved a pixel, the course's dark rim now showing under its
  front edge.) Maintainer 2026-09-18: "same
