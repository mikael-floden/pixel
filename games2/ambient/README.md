...
- **`runtime/scenery.ts` — `sceneryInView(view, categories)`** is how an
  ...
  **EVERY PIECE IS IN THAT LIST TWICE and the seam pairs them** — a base image
  on the painter line and an opaque LIT COPY at `litDepth` (~900_001+), sharing
  a key, a frame and a destination box exactly (measured at the marsh, Day and
  Night: every group is a clean pair). Unpaired, the list said 20 waterline
  pieces where the world had drawn 10 and a population of one per three came
  out double.
  **AND THE PAIR IS WHY `litDepth` IS PUBLISHED: an attached mark must sort
  against the PIECE, never against the band.** Every ambient mark sits at
  ~900_000.0x, just over the darkness overlay — right for something lying on
  the ground, and exactly wrong for something that hovers over a reed and
  perches on it. The dragonflies were painted under their own reeds until
  2026-09-21 (maintainer: "I have never seen a dragonfly ever in this game";
  measured at the marsh, dragonfly 900_000.090 under a reed copy at
  900_001.053) — the same trap the sparks and the moths paid for on 2026-09-09,
  and the same fix: `litDepth + SRC_LIFT` (1e-6), `ABOVE_LIT` (900_001.3) when
  there is no copy. It is a SORT, not an override: the lit band compresses
  painter depth by 1e-5, so a player standing in front of the reeds still draws
  over the dragonfly. Sort among your own marks by an INDEX, never by `y` — a
  y-scaled bias is 5.3e-3 at the marsh and would jump a fly in front of the
  next reed along.
...
| `drips/` | field (INDOOR) | CAVE DRIPS — a POINT THAT KEEPS DRIPPING, not a rain of drops: a drop swells out of the dark above the walls, hangs, lets go, falls faster and faster, and lands in a ring on the floor with a flash and two specks; the same spot takes another one a few seconds later (about every 3.9 s per spout, 15.5 a minute — his 2026-09-17 verdict: "I love the ambient cave drip effect so much I want a little bit more of it! It should drop a bit more often! Maybe 50% more often!"). THE ONLY EFFECT HERE THAT LIVES UNDER A ROOF — it multiplies by `1 - ctx.outdoor`, the mirror of every other row | Inside a CAVE only: a cell whose slab is a `cave` deck (`__ml.t3at`, published kind — a cottage that drips is a leak); dry floor on your own terrace; nothing outdoors, ever |
...
| `dragonflies/` | field | THE WATERLINE IN SUMMER, and the deliberate OPPOSITE of the butterflies above it: still, then a straight line at speed, then still again. It HOVERS on one point (a pixel of jitter, never a drift), DARTS in a linear segment that ends DEAD (easing the ends turns it into a bee), and PERCHES on a reed with its wings still OUT — a butterfly folds its wings at rest and a dragonfly never does, which at four pixels is the whole difference. The wings are a BLUR, not frames: at 400 beats a second there is no pose to draw **AND IT SORTS ONTO ITS OWN REED'S LIT COPY** (`litDepth + SRC_LIFT`), because a mark in the ambient band is painted over by the very piece it belongs to — the maintainer had never seen one in the game until 2026-09-21. **GONE IN RAIN IS THE ACTIVE WEATHER, not `env.rain`**: since the zones took over the sheet, `env.rain` is the drawn splash intensity of this view and reads 0 in a downpour whose zone is off screen | The maps2 agent's waterline pieces in view (`reed_beds`, `cattail_clumps`, `water_lily_clumps` — 124 placed), read by category from the display list; outdoors, by DAY, gone in rain and gone in wind |
...
- **A RATE DIAL IS THE WHOLE CYCLE, NOT THE CONSTANT THAT SOUNDS LIKE IT.**
  ...
  it. Asked for "50% more often" (his, 2026-09-17), scaling `PERIOD` by the
  obvious 1/1.5 delivers **1.33x**, measured, because the hang does not shrink
  with it; the cycle has to be solved for instead (5803 ms -> 3862 ms, 1.503x,
  `PERIOD` [2200,6500] -> [1220,3610], `HANG_MS` deliberately untouched because
  the swell is the part he likes). The unit test that already existed could not
  catch this: it asserted the period sits inside its own jittered band, which
  is true for ANY value of `PERIOD`. **A tuning test must be written against
  the quantity the request names** — `drips.test.ts` now measures the CYCLE and
  is checked to fail at both the old value (0.999x) and the naive scaling
  (1.332x).
  There is also a FLOOR: `nextAt` takes `max(doneAt, hang + period)`, so once a
  period drops under `fallMs(h) + SPLASH_MS` (840 ms at the tallest fall) the
  dial silently stops doing anything. Pinned.
