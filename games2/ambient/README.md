...
- **`ventsInView(pad)`** — the third seam added to `WorldScene`, for
  ...
  of the_game's 8 stacks are over a hearth in a NOT_LIT state, so a plume off
  every one of them is wrong six times out of eight.
