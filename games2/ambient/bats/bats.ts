...
import { SheetSpec, applyFog, dirFromVel, flyCell, flyFrame, gradeCritter, queueSheets, sheetsReady, stepFlapDir } from "../runtime/critters";
...
// Flap frames kept per FACING after the maintainer's cull — the row is still
// FLY_FRAMES wide but only the first count[dir] cells hold art. Generated by
// scripts/cull_frames.py; see art-original/cull.json.
const BAT_COUNTS: readonly number[] = flapframes.critters.bat.count;
...
interface Bat {
  ...
  flapT: number;
  frames: readonly number[]; // kept flap frames per facing (see BAT_COUNTS)
  frame: number; // flap frame 0..frames[dir]-1
  bobPhase: number;
  t: number;
}
  ...
  const launchFlock = (ctx: AmbientCtx) => {
    ...
    for (let i = 0; i < n; i++) {
      ...
      bats.push({
        ...
        dir: dir0,
        dirHoldT: 0,
        // Per-frame. TWICE the wingbeat that was actually on screen (maintainer
        // 2026-08-05). Calibrated against the DRAWN rate, not the old constant:
        // stepFlapDir used to advance at most one frame per tick, so the old
        // 8-12ms was pinned to ~60 frames/s at 60fps. Doubling that is ~120
        // frames/s => ~8.3ms, which is why this is not simply "half of 8-12" —
        // that would have been 4x. See the accumulator note in runtime/flap.ts.
        flapMs: 7.5 + rnd() * 2,
        flapT: rnd() * 100,
        frames: BAT_COUNTS,
        // Desync within THIS facing's kept count — past it the cells are culled
        // padding and the bat would spawn invisible.
        frame: Math.floor(rnd() * BAT_COUNTS[dir0]),
    ...
    debug() {
      ...
      return {
        ...
        sample: s
          ...
          : null,
        // Per-bat (dir, flap column) so QA can assert no bat ever indexes a
        // culled cell, and can measure the live wingbeat rate.
        all: bats.map((b) => ({
          dir: b.dir,
          frame: b.frame,
