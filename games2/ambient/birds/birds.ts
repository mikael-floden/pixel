...
// Flap frames kept per (bird type, facing) after the maintainer's cull — the
// rows are still FLY_FRAMES wide but only the first count[dir] cells hold art.
// Generated by scripts/cull_frames.py; see art-original/cull.json.
const FLAP_COUNTS: readonly (readonly number[])[] = Array.from(
  { length: TYPES },
...
interface Bird {
  ...
  flapT: number;
  frames: readonly number[]; // kept flap frames per facing (see FLAP_COUNTS)
  ...
  const launchFlock = (ctx: AmbientCtx) => {
    ...
    for (let i = 0; i < n; i++) {
      ...
      birds.push({
        ...
        // first frame drawn would be culled padding (an invisible bird).
    ...
    debug() {
      ...
      return {
        ...
        all: birds.map((b) => ({
          type: b.type,
          dir: b.dir,
          frame: b.frame, // flap column of the STATE (may lead the drawn cell by a tick)
          // What is ACTUALLY on screen: the sprite's own frame index, and
          // whether it came from the fly sheet or the perched still sheet.
