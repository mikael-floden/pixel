...
## The foley library (`sounds/foley/`) — 422 sets, generated here

Everything the game plays that is not in the catalog above. Generated by
`sounds/pipeline/foley/generate.py` (ElevenLabs; `ELEVENLABS_API_KEY`),
compressed to opus by `to-opus.py`, served at `/assets/sounds/foley/`.

- `foley.json` — the fat manifest: per set, its takes, measured features, QA
  verdict and the whole generation POOL that lost to them. Written by the
  generator. It is a bare `{set: entry}` map, so **never add a layout key to
  it** — consumers iterate it with `Object.entries` and a phantom set throws.
- `index.json` (`pixel-foley-index@1`) — the path-authoritative index: what
  exists, and `root` (where it lives, repo-root-relative). This is what the
  game engine reads; it JOINS `root` rather than hardcoding a location, which
  is what let the library move here at all.
- `to-opus.py --check` is the CI gate: it fails if either manifest names a
  file that is not on disk. That check exists because `foley.json` spent two
  weeks naming 452 takes `.wav` after they shipped as `.ogg`, and nothing
  noticed — the one consumer that would have caught it was reading the disk
  instead.

The pipeline lives under `sounds/pipeline/` so it inherits publish.json's
`^[^/]+/pipeline/` exclusion and never enters the game image; the library
itself ships.
