...
wiki/
  ...
  lib/             modules build.mjs imports (webp-pixels.mjs, releases.mjs)
                   — SHIPS in the image; tools/ deliberately does not
  tools/           the gates (`check-*.mjs`) and helper scripts
  *.json           build caches: art_bounds, first_seen, world_map,
                   release_notes
```
...
just the last 50 gitsha with commint message and date (just so I as an admin
can see more easily what has landed). Also put the agent if you have that
data (you might be able to tell from the folder that was changed)."* So: no
curation, no versions, no grouping by feature. ADMIN-ONLY, like Parameters —
shas and agent names are the factory floor.

- **The rule lives in `wiki/lib/releases.mjs`** (lib, not tools: it has to
  ship in the image). `build.mjs` puts the document on `data.json` as
  `releases` and refreshes the committed `wiki/release_notes.json` wherever
  git can answer.
- **THE IMAGE HAS NO `.git`** — the same wall `first_seen.json` hit — so the
  deploy writes the list into the build context before the image build
  (`nangijala-deploy.yml`, `build-deploy`), and the deployed page lists the 50
  commits ending at the build being served. That checkout is depth 1, where
  `git log -50` returns ONE commit, so the step deepens **blobless** first:
  measured, 60 commits of history costs 0.1 MB over what the tip already
  carries (6.4 vs 6.3 MB). A plain `--deepen` would pull every blob of every
  commit — hundreds of megabytes on an art repo. The step can never fail the
  deploy; the committed file is the fallback.
- **The agent is derived, in this order**: the `coordination/<agent>.json` a
  commit wrote (one writer per board, so this one cannot be wrong), the author
  where the runner set a real name, then a subject opening with a known
  agent's name. The roster is the set of board files, so an agent hired
  tonight is recognised with no code change.
- **A folder never names an agent.** `games2/` alone is worked by ten agents,
  so where none of the three signals answers, the row shows the folders the
  commit changed — his own suggestion, and a fact rather than a guess. Beside
  a named agent the folders are shown only when they add something
  ("wiki-assistant · Boards"), never as the same fact twice.
- Gate: `wiki/tools/check-releases.mjs` — a player gets no rows, no nav entry,
  no start tile and no direct link; every row is the build's own list in the
  build's own order; a pure board commit names its own agent; his icon is
  48x48 drawn at a whole multiple of 48.
