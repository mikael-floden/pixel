...
class Grow:
    ...
    def put(self, piece, x, y, on=None, hflip=False, lit=False, dir=None,
        ...
        if (int(x), int(y)) in self._indoor_now():
            ...
            if (int(x), int(y)) in getattr(self, "no_place", set()):
                ...
                self.fail += 1
                return False
        # THE PIT STAYS CLEAR: nothing stands in the dungeon's doorway, on its
        # stair, on the banks beside it or on the jambs - a tree on the rim
        # hangs its crown over the way down.
        if (int(x), int(y)) in getattr(self, "mouth_clear", ()):
            self.refused["mouth"] = self.refused.get("mouth", 0) + 1
            self.fail += 1
            return False
    ...
    # -- THE DUNGEON UNDER THE FIELD ------------------------------------------
    # (maintainer 2026-09-09: "What I want is a cave where the player walks
    # down into the ground. Not walking into a mountain this time. The
    # dungeon is under the normal world and the player walks down
    # 'underground' to get to the cave/dungeon. What we will show here is
    # that we can have elevation inside a dungeon (a cave doesn't have to be
    # flat). And we can have worlds under level 0." - and: "Think pokemon. A
    # cave usually has some places you can jump down but not up to.")
    #
    # A pit stair cut into open ground, a doorway at its foot, and under the
    # field ONE cave lid at the field's own level S over three chambers at
    # three depths. In door coordinates (door at 0,0; north is -y):
    #
    #   x:   -2..2      3..6       7..12       8..13       level
    #   hall  H                                            S-6  (six storeys: a house)
    #   east         --E--> |v3    A                       S-6 | S-9   a ledge into A
    #   west  H <--W-- s1          A                       S-6 .. S-9  stair 1, back up
    #   south                      A  |v3 (x 11)           S-9 | S-12  a ledge into B
    #   s2                         s2 (x 8,9)  B           S-9 .. S-12 stair 2, back up
    #   door  x -1..1, y 0                                 S-6
    ...
    # stair of one-level steps - so every chamber is reachable by reversible
    # moves and the reach audit's trap rule holds with no special case.
    # THE LID IS ONE DECK OF THICKNESS 0: its underside is then the field
    # itself (the game's deckBot = S) and nothing hangs below it. A thicker
    # lid draws its skirt over the field along its south and east edge,
    # because both renderers paint decks after every cell (measured, thickness
    # 2: a two-storey grey wall standing on the meadow). The doorway draws as
    # a house door: the lid's cap band reaches S-1, the floor is S-6.
    # THE FLOORS NEVER GO BELOW 0: the game clamps an avatar's elevation and a
    # deck's underside at 0 (games2 buildTerrainGrid, WorldScene), so the
    # field stands DUNGEON_MIN_FIELD above the sea and the deepest chamber is
    # at 0. Under level 0 is a constant once games2 lifts the clamps (posted).
    # THE PLAN IS 17 x 15 CELLS (a 21 x 19 box with its margin): the_game's
    # meadows are cut by roads and ponds, and its widest clean flat at 12 is
    # 11 x 30 - a 24 x 18 plan fitted nowhere (measured).
    DUNGEON_DEPTH = 6
    DUNGEON_LEDGE = 3
    DUNGEON_MARGIN = 2       # untouched field around every dungeon cell
    DUNGEON_PIT = 5          # steps down the pit: S-5 .. S-1, then the door at S-6
    DUNGEON_MIN_FIELD = DUNGEON_DEPTH + 2 * DUNGEON_LEDGE
    DUNGEON_FIELD = ("grass", "dark_mud")   # a meadow, not the mountain's rock
    DUNGEON_ROOMS = (        # (x0, y0, x1, y1) inclusive, depth below the hall
        ((-2, -5, 2, -1), 0),      # the hall
        ((3, -4, 6, -4), 0),       # east passage: ends on the ledge over A
        ((3, -1, 4, -1), 0),       # west passage: the hall to stair 1
        ((7, -6, 12, -1), 3),      # chamber A
        ((11, 0, 11, 1), 3),       # south passage: ends on the ledge over B
        ((8, 2, 13, 7), 6),        # chamber B, the deepest
    )
    DUNGEON_STAIRS = (       # runs, FOOT FIRST, (x, y, depth below the hall); two lanes each
        (((7, -1, 3), (6, -1, 2), (5, -1, 1), (4, -1, 0)),
         ((7, 0, 3), (6, 0, 2), (5, 0, 1), (4, 0, 0))),
        (((8, 2, 6), (8, 1, 5), (8, 0, 4), (8, -1, 3)),
         ((9, 2, 6), (9, 1, 5), (9, 0, 4), (9, -1, 3))),
    )
    DUNGEON_LEDGES = (((6, -4), (7, -4)), ((11, 1), (11, 2)))   # (top, foot)
    DUNGEON_DOOR = ((-1, 0), (0, 0), (1, 0))

    def dungeon(self):
        """Dig the dungeon under the nearest field to the spawn that is high
        enough and flat enough: the plan above, the pit stair published as
        three ramp runs, the two inner stairs as two lanes each."""
        D, P, M = self.DUNGEON_DEPTH, self.DUNGEON_PIT, self.DUNGEON_MARGIN
        # the plan's cells in door coordinates: local -> depth below S
        plan = {}
        for (x0, y0, x1, y1), d in self.DUNGEON_ROOMS:
            for y in range(y0, y1 + 1):
                for x in range(x0, x1 + 1):
                    plan[(x, y)] = D + d
        for lanes in self.DUNGEON_STAIRS:
            for lane in lanes:
                for (x, y, d) in lane:
                    plan[(x, y)] = D + d
        for c in self.DUNGEON_DOOR:
            plan[c] = D
        for x in (-1, 0, 1):
            for k in range(1, P + 1):
                plan[(x, k)] = D - k
            plan[(x, P + 1)] = 0                      # the head: the field
        for x in (-2, 2):
            plan[(x, 0)] = 0                          # the jambs
            for k in range(1, P + 1):
                plan[(x, k)] = max(0, D - k - 2)      # banks two above each step
        xs = [c[0] for c in plan]
        ys = [c[1] for c in plan]
        bx0, bx1 = min(xs) - M, max(xs) + M
        by0, by1 = min(ys) - M, max(ys) + M
        # the fit: every cell of the box is one level of meadow, no deck, no
        # house, no floor - prefix sums make the whole grid one pass
        N = NEW
        lv = np.array(self.lvl, dtype=np.int32)
        grd = np.array(self.grd, dtype=np.int32)
        # the plan's own rect is natural ground at one level (a grey speck
        # in a meadow becomes floor like the rest); the margin ring around it
        # may also carry the road - a road past the pit is how it is found
        ok = np.isin(grd, [self.gi[g] for g in self.NATURAL if g in self.gi])
        ok &= lv >= self.DUNGEON_MIN_FIELD
        for dk in self.doc["decks"]:
            for c in dk["cells"]:
                ok[c["y"], c["x"]] = False
        ...
        for (x, y) in list(getattr(self, "floor_cells", {})) + list(getattr(self, "door_cells", ())):
            ok[y, x] = False
        ok_ring = ok | ((grd == self.gi["light_soil"]) & (lv >= self.DUNGEON_MIN_FIELD))
        eqx = np.zeros((N, N), dtype=np.int32)
        eqx[:, 1:] = lv[:, 1:] == lv[:, :-1]
        eqy = np.zeros((N, N), dtype=np.int32)
        eqy[1:, :] = lv[1:, :] == lv[:-1, :]
        ...
        K, L = N - H + 1, N - W + 1          # box origins: rows, columns

        def rect(ps, x0, y0, x1, y1):
            """sum over the box's inclusive sub-rect, for every origin at once"""
            return (ps[y1 + 1:y1 + 1 + K, x1 + 1:x1 + 1 + L] - ps[y0:y0 + K, x1 + 1:x1 + 1 + L]
                    - ps[y1 + 1:y1 + 1 + K, x0:x0 + L] + ps[y0:y0 + K, x0:x0 + L])
        n_ring = rect(psum(ok_ring.astype(np.int32)), 0, 0, W - 1, H - 1)
        n_ok = rect(psum(ok.astype(np.int32)), M, M, W - 1 - M, H - 1 - M)
        n_eqx = rect(psum(eqx), 1, 0, W - 1, H - 1)
        n_eqy = rect(psum(eqy), 0, 1, W - 1, H - 1)
        fits = np.argwhere((n_ring == W * H) & (n_ok == (W - 2 * M) * (H - 2 * M))
                           & (n_eqx == (W - 1) * H) & (n_eqy == W * (H - 1)))
        assert len(fits), "no field is high and flat enough for the dungeon"
        sx, sy = self.doc["spawn"]

        def meadow(o):
            oy, ox = o
            return collections.Counter(
                self.g(ox + x, oy + y) for x in range(bx0, bx1 + 1) for y in range(by0, by1 + 1)
                if self.g(ox + x, oy + y) in self.NATURAL).most_common(1)[0][0]
        # the nearest meadow fit to the spawn (the snowfields fit too: rock)
        sites = sorted(((int(cy) - by0, int(cx) - bx0) for cy, cx in fits),
                       key=lambda o: (o[1] - sx) ** 2 + (o[0] - sy) ** 2)
        best = next((o for o in sites if meadow(o) in self.DUNGEON_FIELD), None)
        assert best, ("no meadow is high and flat enough for the dungeon", len(sites))
        oy, ox = best
        S = int(lv[oy, ox])
        field = meadow(best)
        # dig
        ...
        cells, floor = {}, set()
        for (x, y), d in plan.items():
            c = (ox + x, oy + y)
            cells[c] = S - d
            self.lvl[c[1]][c[0]] = S - d
        for (x0, y0, x1, y1), d in self.DUNGEON_ROOMS:
            for y in range(y0, y1 + 1):
                for x in range(x0, x1 + 1):
                    floor.add((ox + x, oy + y))
        ...
        for lanes in self.DUNGEON_STAIRS:
            for lane in lanes:
                run = [(ox + x, oy + y) for (x, y, _d) in lane]
                floor.update(run)
                runs.append(run)
        door = [(ox + x, oy + y) for (x, y) in self.DUNGEON_DOOR]
        for c in floor | set(door):
            self.grd[c[1]][c[0]] = mud
        for x in (-1, 0, 1):
            runs.append([(ox + x, oy + k) for k in range(0, P + 2)])
        clear = set(door) | {(ox + x, oy + k) for x in (-2, -1, 0, 1, 2) for k in range(0, P + 1)} \
            | {(ox + x, oy + P + 1) for x in (-1, 0, 1)}
        torch = [(ox - 2, oy + P), (ox + 2, oy + P)]
        clear -= set(torch)
        for run in runs:
        ...
        self.dungeon_deck = {"kind": "cave", "level": S, "thickness": 0, "ground": field,
                             "cells": [{"x": c[0], "y": c[1]} for c in sorted(floor)]}
        self.doc["decks"].append(self.dungeon_deck)
        margin = {(ox + x, oy + y) for x in range(bx0, bx1 + 1) for y in range(by0, by1 + 1)
                  if (x, y) not in plan}
        ...
        self.placed += [("dungeon under the field at", f"door ({ox},{oy}), field level {S}"),
                        ...
                        ("dungeon: scenery evicted", gone - len(self.doc["scenery"])),
                        ("dungeon sites that fit", len(fits))]

    def dungeon_field(self):
        """THE LID WEARS THE FIELD. terrace_grounds recolours the meadow
        after the dig (grass at 12 becomes dark mud), and it recolours only
    ...
    def _audit_site(self, d):
        ...
        def touches(c):
            return any((c[0] + dx, c[1] + dy) in cells for dx, dy in ((1, 0), (-1, 0), (0, 1), (0, -1)))
        # a mountain cave's WALL is the rock that meets its floor edge-on; a
        # breach may lower rock two cells off (measured: two cells beside a
        # mouth, cut to the outside's grade by the fixer)
        hurt = [c for c in d["margin"] - widened
                if (self.lvl[c[1]][c[0]] != d["margin_lv"][c] if pit
                    else (touches(c) and self.lvl[c[1]][c[0]] < wall))
        ...
        on = [(p["piece"], p["x"], p["y"]) for p in self.doc["scenery"]
    ...
    CAVE_LANES = ((2, 2), (3, 1))          # corridor and stair width: never one
                                           # (maintainer 2026-09-12, in a one-wide
                                           # stair of Cave III: "A single monster
                                           # will block the entire dungeon!")
    ...
    def _plan_cave(self, r, lo, hi, free, solid, door_xs, rims=None):
        ...
        for _k in range(1, n_rooms):
            ...
            for _try in range(60):
                ...
                offs = [(v[0] * k, v[1] * k) for k in range(lanes)]
                edge = [c for c in edge if all((c[0] + o[0], c[1] + o[1]) in edge for o in offs)]
                if not edge:
                    continue
                ...
                for j, c in enumerate(path):
                    for o in offs:
                        corr[(c[0] + o[0], c[1] + o[1])] = plv[j]
                if not ok(corr, par):
                    continue
                end = path[-1]
                ent = (end[0] + u[0], end[1] + u[1])
                ents = [(ent[0] + o[0], ent[1] + o[1]) for o in offs]
                w, h = self._weighted(self.CAVE_ROOM_W, r), self._weighted(self.CAVE_ROOM_H, r)
                cl = pl + step
                if u[1]:
            ...
            for d in sorted(range(-self.CAVE_RETURN_MAX, self.CAVE_RETURN_MAX + 1), key=lambda d: (abs(d), r())):
                ...
                for _n in range(min(tries, len(starts))):
                    ...
                    if not ok(tmp, ch, same=gl):
                        continue
                    t = run[-1]
                    # the other lanes of the run, where the rock allows them
                    v = (-u[1], u[0]) if r() < 0.5 else (u[1], -u[0])
                    way = dict(tmp)
                    lane_runs = []
                    for k in range(1, lanes):
                        sv = (s[0] + v[0] * k, s[1] + v[1] * k)
                        lane = {(c[0] + v[0] * k, c[1] + v[1] * k): l for c, l in tmp.items()}
                        if sv in ch and ok(lane, ch | set(way), same=gl):
                            way.update(lane)
                            lane_run = [sv] + [(c[0] + v[0] * k, c[1] + v[1] * k) for c in run]
                            lane_runs.append(lane_run if d > 0 else lane_run[::-1])
                    # FLAT FROM THE TOP OF THE RUN, `lanes` WIDE BY CONSTRUCTION:
                    # the search walks blocks of lanes x lanes cells, so the
                    # way it finds is as wide as the rest (a way found one
                    # cell wide had no room beside it once its halo was
                    # checked; maintainer 2026-09-12, in one: "A single
                    # monster will block the entire dungeon!"). A block cell
                    # touches nothing but the way itself and floors at its
                    # own level; a wall one cell thick toward the open air
                    # is a wall.
                    def block(a_):
                        return [(a_[0] + i, a_[1] + j) for i in range(lanes) for j in range(lanes)]

                    def block_ok(a_, taken):
                        bl = set(block(a_))
                        for q in bl:
                            if q in way:
                                continue
                            if q in cells or not free(q[0], q[1], gl):
                                return False
                            for dx in range(-B, B + 1):
                                for dy in range(-B, B + 1):
                                    qq = (q[0] + dx, q[1] + dy)
                                    ...
                                    if qq in way:
                                        # a step beside a flat cell is a fault; a
                                        # step at its corner is a corner (tested
                                        # before the visited skip: a walk along
                                        # the run's own cells let a flat leg lie
                                        # beside a step four down - measured)
                                        if abs(dx) + abs(dy) == 1 and way[qq] != gl:
                                            return False
                                        continue
                                    if qq in bl or qq in taken:
                                        continue
                                    elif qq in cells:
                                        # the room the run leaves from is near
                                        # by construction; a flat cell may not
                                        # touch it edge-on, anything else may
                                        if cells[qq] != gl and not (qq in ch and abs(dx) + abs(dy) != 1):
                                            return False
                                    elif near and qq[1] <= 0 and not solid(qq[0], qq[1], gl):
                                        return False
                        return True
                    starts_ = [(t[0] - i, t[1] - j) for i in range(lanes) for j in range(lanes)]
                    prev, taken, hit, hit_a = {}, set(), None, None
                    dq = collections.deque()
                    for a_ in starts_:
                        if block_ok(a_, taken):
                            prev[a_] = None
                            taken |= set(block(a_))
                            dq.append(a_)
                    if want is not None:
                        fails["way: " + ("no start" if not dq else "searched")] += 1
                    while dq and hit is None:
                        a_ = dq.popleft()
                        for q in block(a_):
                            if way.get(q, gl) == gl and goal(q, gl):
                                hit, hit_a = q, a_
                                break
                        if hit is not None or len(prev) > reach:
                            break
                        for m in ((a_[0] + 1, a_[1]), (a_[0] - 1, a_[1]), (a_[0], a_[1] + 1), (a_[0], a_[1] - 1)):
                            if m in prev or not block_ok(m, taken):
                                continue
                            prev[m] = a_
                            taken |= set(block(m))
                            dq.append(m)
                    if hit is None:
                        if want is not None:
                            fails["way: " + ("exhausted" if len(prev) > reach else "dead end %d" % len(prev))] += 1
                        continue
                    flat, a_ = [], hit_a
                    ...
                    for a_ in flat:
                        for q in block(a_):
                            if q not in way:
                                way[q] = gl
                    cells.update(way)
                    runs.extend(lane_runs)
                    run_cells = [s] + run
                    runs.append(run_cells if d > 0 else run_cells[::-1])
                    self._last_way = (s, u, d, t, hit, hit_a, flat, sorted((c, l) for c, l in way.items() if c not in tmp))
                    return hit, gl, flat, block(hit_a)
            return None
        ...
        for k, ch in enumerate(chambers):
            ...
            if got is None:
                ...
                return None
        if not all(ch & rev() for ch in chambers):
            R_ = rev()
            xs = [c[0] for c in cells]; ys = [c[1] for c in cells]
            runc = {c for run in runs for c in run}
            pic = []
            for y in range(min(ys), max(ys) + 1):
                pic.append("%4d " % y + "".join(
                    (("s" if (x, y) in runc else ("r" if (x, y) in R_ else "U")) + "%2d" % cells[(x, y)]) if (x, y) in cells else "  ."
                    for x in range(min(xs), max(xs) + 1)))
            raise AssertionError("a room the return stair did not reach\n" + "\n".join(pic) + "\nlast way: %r" % (getattr(self, "_last_way", None),))
        ...
        if rims:
            ...
            for k in sorted(order, key=lambda _k: r()):
                ...
                # the mouth is as wide as the way that reaches it: every rim
                # cell of the same grade and facing beside the last block
                door2 = sorted({m for q in last for m in ((q[0] + 1, q[1]), (q[0] - 1, q[1]), (q[0], q[1] + 1), (q[0], q[1] - 1))
                                if m in rims and rims[m] == rims[rim] and m not in cells})[:lanes]
                for m in door2:
                    cells[m] = gl
                doors.append({"cells": door2, "out": rims[rim][1]})
                break
        if not adjacency_ok():
            fails["adjacency"] += 1
            return None
    ...
    def _register_site(self, site):
        ...
        # A MASSIF CAVE'S WALL TAKES A BREACH IF IT STAYS A WALL: the fixer
        # cuts the terraces' stairs where the terrain wants them, and a cave
        # under the plateau covered those spots with its margin - 18 trap
        # cells on the east shoulder, measured. A breach is STAIR_MAX deep at
        # most, so a wall cell standing that plus a climb above the cave's
        # highest floor may be cut. A pit's field is never cut (its audit).
        self.margin_soft = getattr(self, "margin_soft", {})
        if site["kind"] == "mouth":
            hi = max(site["cells"].values())
            for c in site["margin"]:
                self.margin_soft[c] = hi
    ...
    def _keep_out(self):
        """cells within CAVE_APART / 2 of any cave - a planned site's box, or
        any cave lid's cell (the ported cave has no site: a cave dug beside
        its mouth had two wall cells taken by its corridor widening -
        measured)"""
        out = set()
        R = self.CAVE_APART // 2
    ...
    def _mouth_frame(self, X, Y, o, G, head, rock, blocked, keep, top):
        ...
        def solid(fx, fy, l):
            ...
            return (0 <= w[0] < NEW and 0 <= w[1] < NEW
                    ...
                    and not self._back_touched(w[0], w[1], top)
                    and self.lvl[w[1]][w[0]] >= G + l + self.CLIMB + 1)
        return T, free, solid

    def _through_rims(self, rims, rock, blocked):
        """The rims from which a way two cells wide can reach another rim ten
        or more cells off, at that rim's grade, through rock no other cave
        holds and the mountain's back does not touch. Per absolute floor
        level, the 2 x 2 blocks a floor could take are labelled into
        components; a rim is THROUGH-CAPABLE when a component at another
        rim's grade touches both that rim and the rock behind this one. Drawn
        by rock alone, 1,800 tries found no through-cave once the ways were
        two wide (measured): the through-caves had squeezed one cell wide
        along the back."""
        head = min(h for h, _ in self.CAVE_HEAD)
        grades = sorted({G for _, _, G in rims})
        N = NEW
        comp_at = {}
        for F in grades:
            def bok(a):
                for i in (0, 1):
                    for j in (0, 1):
                        x, y = a[0] + i, a[1] + j
                        if not (0 <= x < N and 0 <= y < N) or self.grd[y][x] not in rock \
                                or (x, y) in blocked or self.lvl[y][x] - F < head:
                            return False
                return True
            label, n = {}, 0
            for y in range(N - 1):
                for x in range(N - 1):
                    ...
                    while st:
                        ...
                        for m in ((u[0] + 1, u[1]), (u[0] - 1, u[1]), (u[0], u[1] + 1), (u[0], u[1] - 1)):
                            if m not in label and bok(m):
                                label[m] = n
                                st.append(m)
            comp_at[F] = label

        def comps_touching(c, F, r=1):
            out = set()
            lab = comp_at[F]
        ...
        for k, (c, o, G) in enumerate(rims):
            by_grade[G].append((k, c))
        through = set()
        for k, (c, o, G) in enumerate(rims):
            for G2, others in by_grade.items():
                if abs(G2 - G) > self.CAVE_UP_MAX:
                    continue
                near = comps_touching(c, G2, r=8)
                if not near:
                    continue
                for k2, c2 in others:
                    if k2 == k or max(abs(c2[0] - c[0]), abs(c2[1] - c[1])) < 10:
                        continue
                    if comps_touching(c2, G2) & near:
                        through.add(k)
                        break
                if k in through:
                    break
    ...
    def mountain_caves(self):
        ...
        for w in self.doc["walls"]:
            # a HOUSE blocks; the terrain's own wall groups are the shelf
            # faces of the massif, and taking them as walls fenced the rock
            # with one-cell lines no two-wide way could cross (measured)
            if w.get("kind") == "house":
        ...
        through = self._through_rims(rims, rock, blocked)
        self.placed += [("rims that can run through to another", f"{len(through)} of {len(rims)}")]
        r = _rng32(0x3C0A7)
        made = made_two = 0
        for k in range(self.CAVE_MOUTHS):
            ...
            thru = {rims[k][0] for k in through}
            for (X, Y), o, G in cand:
                if want2 and thru and (X, Y) not in thru:
                    continue          # a through-cave is owed: only the rims
                                      # that can pay it, while any remain
                along = FACES[o][0]
                n = 0
                for fx in range(-8, 9):
    ...
    def caves(self):
        ...
        # THE ROCK A CAVE IS DUG INTO is anything at its lid's level or
        ...
        def interior(c, rm):
            return all(rock((c[0] + dx, c[1] + dy), rm) or (c[0] + dx, c[1] + dy) in cave_all
                       for dx in (-1, 0, 1) for dy in (-1, 0, 1))
        widened = 0
        for i, dk in decks:
            floor = {(c["x"], c["y"]) for c in dk["cells"]}
            lid = int(dk["level"])
            rm = self.cave_rock_min[i]
            new = {}
            ...
            for (x, y) in sorted(floor):
                ...
                for n in ((x + 1, y), (x, y + 1)):
                    if n in cave_all or n in new or not rock(n, rm):
                        continue
                    if self.lvl[n[1]][n[0]] != lid or not interior(n, rm):
                        continue
                    new[n] = self.lvl[y][x]   # the passage's own level: a cave
                                              # floor is not one level any more
            for (x, y), l in new.items():
                self.lvl[y][x] = l
    ...
    @staticmethod
    def _chambers(cells):
        """4-connected patches of a cell set: the chambers of one lid."""
        left, out = set(cells), []
        while left:
            seed = left.pop()
            comp, st = {seed}, [seed]
    ...
    def cave_dress(self):
        ...
        for i, dk in enumerate(self.doc["decks"]):
            ...
            rm = self.cave_rock_min.get(i, self.ROCK_MIN)

            def wall(c):
                return c not in cave and (not self.g(*c) or self.lvl[c[1]][c[0]] >= rm)
    ...
    def lights(self):
        ...
        rmin = getattr(self, "cave_rock_min", {})
        clear = getattr(self, "mouth_clear", set())

        def n4(c):
            return ((c[0] + 1, c[1]), (c[0] - 1, c[1]), (c[0], c[1] + 1), (c[0], c[1] - 1))

        def grade(m, i):     # land at the cave's grade beyond its floor: the way in
            return (m not in cave and self.g(*m) and not self.liquid(*m)
                    and self.lvl[m[1]][m[0]] < rmin.get(i, self.ROCK_MIN))
        mouth = sorted(c for c, i in cave.items() if any(grade(m, i) for m in n4(c)))
        torches = self._lit_pool("torch_posts")
        tally["cave mouth cells"] = len(mouth)
        tally["house doors"] = len(getattr(self, "house_doors", []))
        # ONE PAIR OF TORCHES PER CAVE: the mountain's stand in its mouth, the
        # dungeon's on the rim of its pit (the doorway and the stair stay clear)
        ...
        if torches:
            for i, spots in sorted(outs.items()):
                spots = sorted(spots)
                for k, m in enumerate((spots[0], spots[-1]) if len(spots) > 1 else spots):
    ...
    def terrace_grounds(self):
        ...
        # A CAVE FLOOR TOUCHES NOTHING: the lid hides the seam, and counting
        # it recoloured the field over the dungeon grey (its dark-mud
        # chambers took the meadow's own colour, measured)
        cave = getattr(self, "cave_floor", {})
        for i, (lv, cells, dom) in enumerate(ts):
            for (x, y) in cells:
                if (x, y) in cave:
                ...
                for dx, dy in ((-1, 0), (0, -1)):        # north, west of me
                    j = idx.get((x + dx, y + dy))
                    if j is not None and j != i and (x + dx, y + dy) not in cave \
    ...
    def _back_touched(self, x, y, top, lvl=None):
        """True where mountain_back will cut or void the cell: behind the
        crown AND either a shoulder below CROWN_MIN (cut to the valley) or
        hidden under the ridge (voided). Rock at the crown's own height that
        still shows above the ridge is left alone, and a cave may be dug
        under it (blocking everything up-screen of the crown pinched the
        massif's east band to one cell and no way two cells wide reached a
        second mouth - measured)."""
        if not self._behind_crown(x, y, top):
            return False
        l = self.lvl[y][x] if lvl is None else lvl
        return l < self.CROWN_MIN or self._under_ridge(x, y, l, top)
    ...
    def mountain_back(self):
        ...
        for dk in self.doc["decks"]:
            if dk.get("kind") == "cave":
                for c in dk["cells"]:
                    lidlv[(c["x"], c["y"])] = max(lidlv.get((c["x"], c["y"]), 0), int(dk["level"]))
        hid = [c for c in cave if self._back_touched(c[0], c[1], top, lidlv.get(c, self.lvl[c[1]][c[0]]))]
        assert not hid, ("a cave floor lies where the back is cut or voided", len(hid), hid[:6])
    ...
    def way_ground(self):
        ...
        # side and the dungeon's pit is three, and rolling per run striped
        # the pit light_soil | dark_mud | grey_paving_stone (measured). Runs
        # that touch are one stair, one roll, one material.
        stairs = [r for r in self.doc.get("ramps", []) if r.get("kind") == "stair"]
        owner = {}
        parent = list(range(len(stairs)))
        ...
        def find(i):
            while parent[i] != i:
                ...
                i = parent[i]
            return i
        for i, r in enumerate(stairs):
            for c in r["cells"]:
                x, y = c["x"], c["y"]
                for m in ((x, y), (x + 1, y), (x - 1, y), (x, y + 1), (x, y - 1)):
                    j = owner.get(m)
                    if j is not None and find(i) != find(j):
                        parent[find(i)] = find(j)
                owner[(x, y)] = i
        ...
        for group in groups.values():
            cells = [(c["x"], c["y"]) for r in group for c in r["cells"]]
            cellset = set(cells)
            seed = min(cellset)
            rr = _rng32((seed[0] * 2654435761 ^ seed[1] * 40503 ^ 0x57A12) & 0xffffffff)
            ...
            for r in group:
                r["ground"] = mat
            kinds[f"{why}: {mat}"] += 1
        # BUILD ASSERT: touching stair runs wear one material
        for r in stairs:
            for c in r["cells"]:
                for m in ((c["x"] + 1, c["y"]), (c["x"], c["y"] + 1)):
                    j = owner.get(m)
                    assert j is None or stairs[j]["ground"] == r["ground"], ("striped stair", c, m)
    ...
    def _dissolve_specks(self):
        ...
        takes the ground its rim mostly is - the rim ON ITS OWN PLANE, within
        a step: a cliff is another plane, and rock ringed by cave floors
        several levels down is a wall, not a speck (measured: a two-cell wall
        between two cave floors turned mud). Repeats until nothing changes -
        dissolving one speck can expose another."""
        total = 0
        for _ in range(6):
            n = 0
            for g0, l0, comp in list(self._components(
                ...
                for x, y in rim:
                    gg = self.g(x, y)
                    if gg in self.NATURAL and gg != g0 and abs(self.lvl[y][x] - l0) <= 1:
                        votes[gg] += 2 if self.lvl[y][x] == l0 else 1
                if not votes:            # a mud dot in a road becomes road
                    for x, y in rim:
                        gg = self.g(x, y)
                        if self._speck_rim(x, y) and gg != g0 and abs(self.lvl[y][x] - l0) <= 1:
                            votes[gg] += 1
    ...
    def audit_ground(self):
        ...
        # a speck has a rim on its own plane to dissolve into (a step at
        # most, as _dissolve_specks reads it); rock ringed only by floors
        # several levels down is a wall, a rock in the sea an islet
        specks = [comp for g0, l0, comp in self._components(
            lambda x, y: self.g(x, y) in self.NATURAL, by_level=False)
            if len(comp) <= self.SPECK_MAX
            and any(self._speck_rim(x, y) and abs(self.lvl[y][x] - l0) <= 1 for x, y in self._rim(comp))]
    ...
    def _standable_after_back(self):
        ...
        for (x, y, layer) in list(lv):
            if layer == 0 and self.lvl[y][x] > self.SHOULDER_MAX and self._back_touched(x, y, top):
                del lv[(x, y, layer)]
        return lv
    ...
    def _carvable(self, x, y):
        g = self.g(x, y)
        # A CAVE FLOOR NEVER WANTS A STAIR TO THE SURFACE - its lid is the
        # surface. The slope-top rule read the dungeon's inner stair as a
        # slope ending under a cliff and cut a breach from its head up
        # through the field (measured: a 5-step light_soil stair beside the
        # pit). The pit itself is cut once, by dungeon().
        soft = getattr(self, "margin_soft", {})
        return (g and g not in self.INDOOR_GROUNDS and g != "light_soil"
                ...
                and ((x, y) not in getattr(self, "dungeon_margin", set())
                     or ((x, y) in soft and self.lvl[y][x] - self.STAIR_MAX >= soft[(x, y)] + self.CLIMB + 1)))
            ...
            for T in comps:
                ...
                for _, t, b in cands:
                    ...
                    if where:
                        laid.append(t)
                        fixed["breach"] += 1
                # a trap this small is a ledge and joins the terrace above -
                # and so does any trap up to a pocket's size once every
                # breach has failed to land (the terrace above is a cave's
                # lid, or too narrow for a stair and its flare): the east
                # shoulder's 18-cell strip beside the through-cave, measured
                # as 18 trap cells and a red build. A hole is not terrain.
                if not laid and len(T) <= self.POCKET_MAX:
                    _, t, b = cands[0]
                    ...
                    for (x, y) in T:
                        ...
                        if not self.liquid(x, y):
                            self.grd[y][x] = gb
                    # a stair inside a trap led nowhere; raised, its steps
                    # would break the ramp contract (measured: one run)
                    before = len(self.doc.get("ramps", []))
                    self.doc["ramps"] = [rr for rr in self.doc.get("ramps", [])
                                         if not any((c["x"], c["y"]) in T for c in rr["cells"])]
                    fixed["runs dropped by a join"] += before - len(self.doc["ramps"])
                    fixed["joined"] += 1
                    laid.append(t)
                progress += len(laid)
            if not progress:
    ...
    def ramps(self):
        ...
        # the road's runs are rebuilt from the road; a CUT stair stays (the
        # dungeon's pit and inner stairs are published before this pass, and
        # rebuilding from the road alone dropped all seven: the lanes then
        # kept the meadow's grass and the way down was never a ramp)
        self.doc["ramps"] = [r for r in self.doc.get("ramps", []) if r.get("kind") == "stair"] + runs
