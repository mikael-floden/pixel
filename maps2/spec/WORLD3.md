...
### the roads: one width on screen

**A road is as wide on screen whichever way it runs** (maintainer
2026-09-23, beside the road at 247,240: *"The road is way more narrow when
you draw it horizontally vs vertical. Can you try to maintain the same road
width?"*). `widen_roads` is a floor in CELLS — three, so the corner-Wang
blend has a solid 2x2 quad — and three cells is not one width: a cell is a
64x28 diamond, so three rows along a cell axis stand 77 px across on screen,
three anti-diagonals of a screen-horizontal staircase 56 px, three diagonals
of a screen-vertical one 128 px (and the solid core 51, 28 and 64 px). So
width is measured in PIXELS across the road's direction on screen
(`roadwidth.py`: the direction from a PCA of the road's screen positions
round the cell, the width the median of five walks across the band along
...
thinned: a screen-vertical road is wider than its reference by the
projection, and thinning breaks the solid core. A cell narrower than 50 px
...
| walls | `tiles/review` x-over-y matrix | **the only tiles that ever show a wall.** A column stacks whole tiles (the tiles agent's `plateau` model): same-over-same for every storey below, capped by `top__over__side` where `side` = the ground at the face's FOOT (down-screen lower neighbour) — never an indoor floor, never a liquid — overridable per pair via `live/tuning/tile_walls.json`. Candidates = the wiki's own rule: maintainer-approved first, else rank 0. WHICH approved tile a course wears is the game's **wall region field** (`games2/client/src/wallregion.ts`, ported into render3 and proved against its `WALL_TEST_VECTORS` at import): a domain-warped 3-D region about 40 cells across picks ONE measured set of five joining tiles from `games2/client/src/wallsets.json` (expected seam cost ≤ 2.0 under the 8/5/3/2/1 mix; no such set → rank 0 alone), and a weighted member per cell AND storey — per storey, never per column, so a boundary crosses a face instead of running along it. A caller with no cell (the flat-tile ladder, a borrowed wall, the pitch probe) gets rank 0. |
...
| boundaries | `tiles/patterns` x `tiles/plates` | patterns publishes the **material-independent** Wang boundary and nothing else; the two grounds it divides come from their own set members. So **every pair is covered**, including roads (`light_soil` beside `grass`, the 2nd most common boundary on the_game) — no per-pair set is required. Corner lattice, index `8*NW+4*NE+2*SW+1*SE`; each half asks for **its own ground's** region. Only where the quad shares one level. **WATER LIES FLAT**: a corner within one storey votes its own ground (the game's `BOUNDARY_STEP`, so a terrace rim still blends), but a LIQUID corner votes only at the cell's OWN level — one storey of tolerance let the sea compose into the top face of the step above it, water running up a stair on a cell a whole level clear of it (maintainer 2026-09-11, ringing the bottom step of a shore staircase: *"The ground on that stair has fucking water on it!"*; 8 cells of the_game, every one land at level 1 beside water at 0). A WATER cell still composes its land corner, which is the shore tile that is "not 100% water or 100% beach". **tiles3 carries the unamended rule** and the parity fixture pins the old stream — raised on the games2 board. |
| fades | `tiles/fades` (`tiles3/fade-tiles@1`) | top-only mix tiles that warm the player up for a ground change **before** the switch. Placed by `edge_ground`, never by area majority ("big rocks ON an ice sheet"). **APPROVED ONLY** — he rates this layer actively (480 approved, 345 rejected of 3,575), so an unjudged tile is not a candidate; survivors are weighted by his rating. A **scattered event** over a real Chebyshev distance band, never a coat of one tile. The share that orders a pool and steers the falloff is **`area_pct`, the measured top-face area** — not `pct`, which is the producer's placement label (`pct[edge] = 51 + 49*area_edge`, built so the edge winner is always the majority, so `pct[other]` is 0.49 x the real area and never exceeds 49). Ordering is the same either way (monotone), but the number the dial interpolates over is the real one now. **Floor 1%, no ceiling**: the old `<= 55` could never fire under `pct`, and on the real area it would cut 725 tiles across 120 of the 207 pools and empty two — an INVERTED tile (area majority rock, rim ice) is valid and places on ice, his own 2026-08-28 ruling. Every pool is identical to the one that shipped; what moves is which tile the falloff picks inside it. **The game still reads `pct`** (`tiles3.ts fadeTier`), so until it makes the same switch the parity fixture cannot be regenerated — posted to games. |
| details | `live/feedback/tiles.json` `<key>#top` approvals, then `tiles/tops/index.json` | **The x-over-y `#top` approvals first, then his detail LIBRARY** — every `flavour: detail` sheet's tile whose `<dir>/<tile>#top` verdict is approved (2,549 of 9,840 on 2026-09-12), drawn as its `post/` file matched by stem, never a constructed name. Index order, no existence check (the index is gated to 0 dangling files; a miss is loud, a shorter pool would shift every index after it) — the game's `detailPool` (tiles3.ts) builds the same list in the same order, so the per-cell roll `(x*83492791) ^ (y*2654435761) ^ 0xd47a` at `DETAIL_FREQ` **1/100** — his dial's default (games2 `detailrate.ts`, 2026-09-13; 1/56 was the pre-slider rate, and render3 kept it six days too long) — lands on the same tile in both, under the same three clauses: never on the room floor, **never on a rise** (a cell its own ground rises beside — the game's `slopeIndexAt` mask, where it draws its ramp), **never touching another** (`detail_alone`: among the raw winners of an 8-ring the smallest roll keeps its detail; a neighbour on a rise never vetoes, a neighbour a fade took still does). A tile **rejected as a pair** (bad wall) can still be a top-approved detail — the two reviews are independent by design. Conformed, so a detail's foreign lava/ice/sand wall never leaks into a field; never on `parquet_floor`. |
| slopes | none | **The slope tile library is retired** (maintainer 2026-09-26: the game composes each ramp from the cell's own plate, dynamically; `tiles/slopes` is deleted). render3 draws a rise as the plain stair it is in the data, and does not reproduce the game's ramp: a change page shows the step, the game shows the incline. The rise mask survives only as the detail veto. |
