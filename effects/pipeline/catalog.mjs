...
//
//   effects/library/index.js   one import per effect file (what the viewer and
//                               the game import)
//   effects/effects.json        the catalog: every effect's metadata, no code
//                               (what the wiki lists and a tooltip reads)
//
//   node effects/pipeline/catalog.mjs           write both
//   node effects/pipeline/catalog.mjs --check   exit 1 if either is stale
//
...
async function main() {
  ...
  for (const rel of list) {
    ...
    entries.push({
      id: d.id,
      key: `effects/library/${d.id}`,
      file: `effects/library/${rel}`,
      name: d.name,
      kind: d.kind,
  ...
  const catalog =
    JSON.stringify(
      {
        format: "nangijala-effects-catalog@1",
        runtime: "effects/runtime/index.js",
        viewer: "effects/viewer/index.html",
        families: FAMILIES,
  ...
  const targets = [
    ...
    [join(DOMAIN, "effects.json"), catalog],
  ];
  let stale = 0;
  for (const [path, text] of targets) {
    ...
    stale++;
    if (CHECK) console.error(`stale: ${path} (run node effects/pipeline/catalog.mjs)`);
