...
tunes it. The shaders domain ships a finished review surface for the LOOK
(the viewer); the wiki owns the section, the verdicts and the save.

## What there is to show

- **The catalog** `shaders/shaders.json` (`nangijala-shaders-catalog@1`,
  ...
  `shaders/library/<id>`), `file`, `name`, `kind` + `kindLabel`, `family`,
...
- **The viewer** `shaders/viewer/index.html`: the stage (the game's iso floor,
  ...
  `/assets/shaders/viewer/` once games ships the domain (raw.githubusercontent
  answers text/plain and a module will not load from it).

## Embedding

`<iframe src="/assets/shaders/viewer/index.html?embed=1&id=fire/fireball&level=5">`
...
|---|---|---|
| viewer -> wiki | `{ type: "shaders:ready", ids }` | booted; the library it has |
| viewer -> wiki | `{ type: "shaders:selected", id, key, level }` | he picked another effect in the viewer |
| viewer -> wiki | `{ type: "shaders:tune", id, key, values, defaults }` | a tunable changed: `values` = everything that differs from the default now (empty = back to defaults) |
| wiki -> viewer | `{ type: "shaders:open", id, level? }` | show this effect |
| wiki -> viewer | `{ type: "shaders:tuning", table }` | the live `overrides` table: it becomes what the sliders start from |
...
- **Verdicts** `live/feedback/shaders.json` — `pixel-wiki-feedback@1`, domain
  ...
  level is the effect's own range, not a facet). The shader agent reads it
...
`games2/server/src/live.ts`) — asked of games on the shaders board.
...
text (also inside the viewer), and the save bar picking up `shaders:tune`.
